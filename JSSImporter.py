...
class JSSImporter(Processor):
    ...
    input_variables = {
        ...
        "website": {
            "required": False,
            "description": "Website for self service description",
        },
    }
    output_variables = {
        "jss_category_added": {
    ...
    def main(self):
        ...
        self.env["selfserve_policy"] = prod_name + " " + version
	...
	#
        ...
        if self.env.get("selfserve_policy"):
            ...
            apiUrl = "policy"
            proceed_list = self.checkItem(repoUrl, apiUrl, item_to_check, base64string)
            template_string = """<?xml version="1.0" encoding="UTF-8"?><policy><general><name>%selfserve_policy%</name><enabled>true</enabled><trigger>USER_INITIATED</trigger><trigger_other/><frequency>Ongoing</frequency><target_drive>/</target_drive><offline>false</offline><category><id>%cat_id%</id><name>%CAT_NAME%</name></category></general><scope><computer_groups><computer_group><id>%grp_id%</id></computer_group></computer_groups></scope><self_service><use_for_self_service>true</use_for_self_service><self_service_description>Install %selfserve_policy% (&lt;a href="%website%"&gt;%website%&lt;/a&gt;)&lt;br&gt;&lt;br&gt;IMPORTANT: Close all running copies of the application before proceeding!</self_service_description><force_users_to_view_description>true</force_users_to_view_description></self_service><package_configuration><packages><size>1</size><package><id>%pkg_id%</id><action>Install</action></package></packages></package_configuration><maintenance><recon>true</recon></maintenance></policy>"""
	    self.output(item_to_check)
            self.output("Current grp_id is %s, pkg_id is %s, cat_id is %s" % (grp_id, pkg_id, cat_id))
            replace_dict = {"%PROD_NAME%" : prod_name, "%grp_id%" : grp_id, "%pkg_id%" : pkg_id, "%selfserve_policy%" : self.env.get("selfserve_policy"), "%cat_id%" : cat_id, "%CAT_NAME%" : category_name, "%website%" : website }
	    self.output(proceed_list)
            if "proceed" not in proceed_list:
